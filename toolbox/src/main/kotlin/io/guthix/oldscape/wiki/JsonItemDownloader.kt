...
@KtorExperimentalAPI
fun main() {
    val downloader = JsonItemDownloader()
    val itemconfigs = downloader.load()
    val wikiConfigsJson = Json(downloader.config).stringify(ItemWikiDefinition.serializer().list, itemconfigs)
    val path = Path.of(JsonItemDownloader::class.java.getResource("/").toURI())
    Files.writeString(path.resolve("wikiNpcConfigs.json"), wikiConfigsJson)
}

class JsonItemDownloader : JsonDownloader() {
    @ExperimentalCoroutinesApi
    @KtorExperimentalAPI
    fun load() = runBlocking<List<ItemWikiDefinition>> {
        ...
        for(config in cacheConfigs) {
            if(config.id !in idsScraped) {
                ...
                val definitions = parseWikiString<ItemWikiDefinition>(wikiText ?: "")
